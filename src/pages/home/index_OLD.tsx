import { ArrowDownOutlined, CheckCircleOutlined, CheckOutlined, ClockCircleOutlined, LoadingOutlined, LogoutOutlined, SendOutlined } from "@ant-design/icons";
import { Client, IMessage } from '@stomp/stompjs';
import { Button, Col, Divider, Form, Layout, Row, Tag, Tooltip, notification } from "antd";
...
export const HomePage = () => {
  ...
  const [creatingQueue, _setCreatingQueue] = useState<boolean>(false)
  const [consumer, setConsumer] = useState<Client>()
  const [loadingMessages, setLoadingMessages] = useState<boolean>(false)
  const [_publishingMessage, _setPublishingMessage] = useState<boolean>(false)
      ...
      <Layout style={{ height: '90vh' }}>
        <Content>
          <Layout style={{
            ...
            <Content ref={contentRef} style={{ alignContent: "flex-end", overflowY: "auto" }}>
              {(loadingMessages || creatingQueue) &&
                <div style={{ textAlign: "center", fontStyle: "italic", color: "#ccc" }}>
                  Carregando mensagens... <LoadingOutlined />
                </div>
              }
                          ...
                          <>
                            <span style={{ color: "#ccc", textAlign: "end", fontStyle: "italic", display: "flex", float: "inline-end", gap: "4px" }}>
                              ...
                              <Tooltip title={StatusChatMessageTranslate[chatMessage.status]}>
                                {/* {chatMessage.receivements?.length > 0 ? <CheckCircleOutlined style={{ color: "skyblue" }} /> : <ClockCircleOutlined />} */}
